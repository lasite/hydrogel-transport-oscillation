...
# -*- coding: utf-8 -*-
"""
linear_stability_1d.py — Homogeneous stability diagnostic for the 1D slab model.

Two complementary analyses:
  1. Analytical homogeneous/lumped diagnostic σ(k) around a spatially
     uniform steady state.
     The 3×3 eigenvalue problem A(k)·v = σ·v captures:
       - k=0: homogeneous (0D-equivalent) Hopf instability
       - k>0: local bulk dispersion corrections

This is not the full spectrum of the linearized Robin-boundary finite-volume
PDE. It combines a lumped homogeneous balance, including boundary exchange
terms, with local bulk dispersion corrections and is used as a necessary but
not sufficient indicator of possible oscillatory feedback.
...
def normalized_model_version(p) -> str:
    ...
    mv = aliases.get(str(getattr(p, "model_version", "v0")), None)
    if mv is None:
...
def porosity_fields(J, p):
    ph = phi_of_J(J, p)
    por = np.clip(1.0 - ph, 0.0, 1.0)
    phi_ref = p.phi_p0 / max(p.J_init, p.phi_p0 * 1.02)
    por_ref = max(1.0 - phi_ref, 1.0e-12)
    return ph, por, np.clip(por / por_ref, 0.0, 1.0)
...
def chem_pot(J, theta, p):
    ...
    ph = phi_of_J(J, p)
    chi = p.chi_inf + p.S_chi * theta + p.chi1 * ph
    m_mix = np.log(1 - ph) + ph + chi * ph**2
    if uses_strict_free_energy_chi(p):
        m_mix = m_mix - ph**2 * (1.0 - ph) * p.chi1
...
def reaction_rate(u, theta, J, p):
    ...
    ph, _, norm_por = porosity_fields(J, p)
    if is_v1_model(p):
        act = _v1_porosity_closure(norm_por, ph, p.m_act, p.A_min, p)
    else:
        act = np.maximum(1.0 - ph, 1.0e-12)**p.m_act + p.a_floor
    return np.maximum(u, 1.0e-12)**p.reaction_order * act * thermal_factor(theta, p)


def mobility_ref(J, theta, p):
    ph, _, norm_por = porosity_fields(J, p)
    if is_v1_model(p):
        base = norm_por**p.m_mob + p.mu_floor
    else:
        base = np.maximum(1.0 - ph, 1.0e-12)**p.m_mob + p.mu_floor
    return p.M0 * base
...
def diffusivity_ref(J, theta, p):
    ph, _, norm_por = porosity_fields(J, p)
    if is_v1_model(p):
        base = _v1_porosity_closure(norm_por, ph, p.m_diff, p.D_min, p)
    else:
        base = np.maximum(1.0 - ph, 1.0e-12)**p.m_diff + p.d_floor
...
def conductivity_ref(J, p):
    if uses_metric_corrected_transport(p):
        return p.K0 / np.maximum(J, 1.0e-12)
    return p.K0


def heat_capacity_ref(J, p):
    if uses_metric_corrected_transport(p):
        return p.C0 * np.maximum(J, 1.0e-12)
    return p.C0
...
def df_dJ(J, theta, p):
    ...
    if is_v1_model(p):
        h = max(1.0e-6, 1.0e-6 * abs(J))
        return (chem_pot(J + h, theta, p) - chem_pot(J - h, theta, p)) / (2.0 * h)
    ph = phi_of_J(J, p)
...
def dR_du(u, theta, J, p):
    ...
    return p.reaction_order * R0 / max(u, 1.0e-12)
...
def dR_dJ(u, theta, J, p):
    ...
    if is_v1_model(p):
        h = max(1.0e-6, 1.0e-6 * abs(J))
        return (reaction_rate(u, theta, J + h, p) - reaction_rate(u, theta, J - h, p)) / (2.0 * h)
    ph = phi_of_J(J, p)
    R0 = reaction_rate(u, theta, J, p)
...
def build_A0(J0, u0, theta0, p):
    ...
    # dθ/dt = [-Bi_T·θ + Da·J·R] / C_ref
    A0[2, 0] = p.Da * (R0 + J0 * RJ) / Cref
    A0[2, 1] = p.Da * J0 * Ru / Cref
    A0[2, 2] = (-p.Bi_T + p.Da * J0 * Rt) / Cref
...
def build_D2(J0, u0, theta0, p):
    ...
    fJ = df_dJ(J0, theta0, p)
    ft = df_dtheta(J0, theta0, p)
    M0 = mobility_ref(J0, theta0, p)
    D_eff = p.delta * diffusivity_ref(J0, theta0, p)
    K_eff = p.alpha * conductivity_ref(J0, p)
    C_eff = heat_capacity_ref(J0, p)

    D2 = np.zeros((3, 3))
    D2[0, 0] = -M0 * fJ           # J diffusion via chemical potential
    D2[0, 2] = -M0 * ft           # J-θ coupling via χ(θ)
    D2[1, 1] = -D_eff / J0        # reactant diffusion
    D2[2, 2] = -K_eff / C_eff   # thermal diffusion
    return D2
...
if __name__ == "__main__":
    p = LSAParams()
    print("=" * 60)
    print("1D Gel Slab - Homogeneous Stability Diagnostic")
    print("=" * 60)
