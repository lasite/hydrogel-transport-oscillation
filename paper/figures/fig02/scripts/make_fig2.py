"""
make_fig2.py — Composite tiler for Fig 2 (2×2 mosaic).

Two outputs:
  * ``fig2.pdf`` — VECTOR composite, built by ``pypdf`` from the
    per-panel PDFs. Heatmaps stay raster (rasterized=True in panels);
    line art and text remain vector.
  * ``fig2.png`` — raster composite from the per-panel PNGs.
...
from fig2_style import FIG_DIR, PANEL_W, PANEL_H, DPI_PNG
...
def _build_png():
    ...
    out_png = os.path.join(FIG_DIR, "fig2.png")
...
def _build_pdf():
    ...
    out_pdf = os.path.join(FIG_DIR, "fig2.pdf")
...
def _publish():
    """Copy the composite into Figure/pub/ where main.tex's
    \\graphicspath looks for it."""
    import shutil
    pub_dir = os.path.normpath(os.path.join(_HERE, "..", "Figure", "pub"))
    os.makedirs(pub_dir, exist_ok=True)
    for ext in ("pdf", "png"):
        src = os.path.join(FIG_DIR, f"fig2.{ext}")
        dst = os.path.join(pub_dir, f"fig2.{ext}")
        shutil.copyfile(src, dst)
    print(f"  Published to: {pub_dir}/fig2.{{pdf,png}}")
