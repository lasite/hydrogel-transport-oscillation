"""
make_fig2a.py — Panel (a): regime classification on (Bi_T, S_chi).

Cache: data/fig2/panel_a.npz  (subset of data/fig4/fig4_grid_Bi_T_S_chi.npz)
Render: Figure/fig2/panel_a.{pdf,png}

The upstream 25×25 grid is built by ``fig4_data.build_grid`` (24-worker
PDE sweep, hours of compute). This panel only re-uses it, then writes the
single 2-D ``regime`` field needed for rendering.
"""
import os, sys
import numpy as np
import matplotlib
...
def compute():
    ...
    np.savez(CACHE,
             Bi_T=g["x"], S_chi=g["y"], regime=g["regime"],
...
def render():
    ...
    _categorical_pcolormesh(ax, d["Bi_T"], d["S_chi"], d["regime"])
    ...
    # In-panel legend showing only regimes actually present.
