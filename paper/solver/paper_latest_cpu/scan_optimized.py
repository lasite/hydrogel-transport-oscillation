...
class Params:
    ...
    # Additive floors on (1-phi)^m constitutive terms.  Physical motivation:
    # PNIPAM gels show non-zero residual transport in the collapsed state
    # (collapsed/swollen diffusivity ratio ~ 1e-3..1e-2), not the (1-phi)^m → 0
    # limit.  Numerically, floors bound the Jacobian condition number from
    # ~1/(1-phi)^m ~ 1e12 at phi→1 down to ~1/floor.  Defaults are 0 so
    # existing behavior / cached results are unchanged.
    a_floor: float = 0.0   # activity floor: a = (1-phi)^m_act + a_floor
    d_floor: float = 0.0   # diffusivity floor: D = D0 * ((1-phi)^m_diff + d_floor)
    mu_floor: float = 0.0  # mobility floor: M = M0 * ((1-phi)^m_mob + mu_floor)
    A_min: float = 1.0e-4  # v1 normalized accessibility floor
    D_min: float = 1.0e-4  # v1 normalized diffusivity floor
    closure_form: str = "power"  # power | mackie_meares | free_volume
...
def make_jac_sparsity(N: int) -> csc_matrix:
    ...
    Block bandwidth:
      ...
      (theta,J/W):   1/0 — heat source and metric-corrected coefficients
    """
    size = 3 * N
...
def normalized_model_version(p) -> str:
    aliases = {
        "v0": "v0",
        "model_v0": "v0",
        "v0_legacy": "v0",
        "v1": "v1_first_principles",
        "model_v1_first_principles": "v1_first_principles",
        "v1_first_principles": "v1_first_principles",
        "model_v1_effective_chi_optional": "v1_effective_chi_optional",
        "v1_effective_chi_optional": "v1_effective_chi_optional",
        "model_v1_metric_corrected_optional": "v1_metric_corrected_optional",
        "v1_metric_corrected_optional": "v1_metric_corrected_optional",
    }
    mv = aliases.get(str(p.model_version), None)
    if mv is None:
        raise ValueError(f"unknown model_version={p.model_version!r}")
    return mv


def is_v1_model(p) -> bool:
    return normalized_model_version(p).startswith("v1_")


def uses_strict_free_energy_chi(p) -> bool:
    return normalized_model_version(p) in {
        "v1_first_principles",
        "v1_metric_corrected_optional",
    }


def uses_metric_corrected_transport(p) -> bool:
    return normalized_model_version(p) == "v1_metric_corrected_optional"


def porosity_fields(J, p):
    phi = phi_from_J(J, p)
...
def _v1_porosity_closure(norm_porosity, phi, exponent, floor, p):
    ...
    if form == "mackie_meares":
        ...
        ratio_ref = (1.0 - p.phi_p0 / max(p.J_init, p.phi_p0 * 1.02)) / (
            1.0 + p.phi_p0 / max(p.J_init, p.phi_p0 * 1.02)
        )
...
def accessibility_factor(J, p):
    phi, _, norm_porosity = porosity_fields(J, p)
    if is_v1_model(p):
        return _v1_porosity_closure(norm_porosity, phi, p.m_act, p.A_min, p)
    return np.maximum(1.0 - phi, 1e-12)**p.m_act + p.a_floor


def mobility_ref(J, theta, p):
    phi, _, norm_porosity = porosity_fields(J, p)
    if is_v1_model(p):
        base = norm_porosity**p.m_mob + p.mu_floor
    else:
        base = np.maximum(1.0 - phi, 1e-12)**p.m_mob + p.mu_floor
    return p.M0 * base


def diffusivity_ref(J, theta, p):
    phi, _, norm_porosity = porosity_fields(J, p)
    if is_v1_model(p):
        base = _v1_porosity_closure(norm_porosity, phi, p.m_diff, p.D_min, p)
    else:
        base = np.maximum(1.0 - phi, 1e-12)**p.m_diff + p.d_floor
    D_cur_or_ref = p.D0 * base
    if uses_metric_corrected_transport(p):
        return D_cur_or_ref / np.maximum(J, 1.0e-12)
    return D_cur_or_ref


def conductivity_ref(J, p):
    K_cur_or_ref = p.K0 * np.ones_like(J)
    if uses_metric_corrected_transport(p):
        return K_cur_or_ref / np.maximum(J, 1.0e-12)
    return K_cur_or_ref


def heat_capacity_ref(J, p):
    C_cur_or_ref = p.C0 * np.ones_like(J)
    if uses_metric_corrected_transport(p):
        return np.maximum(J, 1.0e-12) * C_cur_or_ref
    return C_cur_or_ref
...
def local_chem_pot(J, theta, p):
    ...
    if uses_strict_free_energy_chi(p):
...
def state_fluxes(J, W, theta, p, dx):
    ...
    # swelling flux q, positive in the increasing-xi direction
    q = np.zeros(p.N + 1)
    ...
    # total reactant flux n = u q - delta D_ref u_x
    ...
    # heat flux h = -alpha K_ref theta_x
    h = np.zeros(p.N + 1)
    ...
    return {
        "u": u, "m_local": m_local, "m": m, "q": q, "R": R,
        "nflux": nflux, "h": h, "C_ref": C_cell,
        "D_ref": D_cell, "K_ref": K_cell, "M_ref": M_cell,
    }
...
def rhs_mol_logJ(t, y, p):
    ...
    logJ_t = -(q[1:] - q[:-1]) / (dx * J) \
             - p.Bi_J_vol * (aux["m_local"] - p.m_b) / J

    # 体积源: B_vol*(J - W) = B_vol*(1 - u_raw)*J, 直接用W(光滑,无截断)
    # 体积均匀供给: Bi_c_vol*J*(1-u) = Bi_c_vol*(J - W)
    nflux = aux["nflux"]
    W_t = -(nflux[1:] - nflux[:-1]) / dx - p.Da * J * aux["R"] \
          + p.B_vol * (J - W) \
          + p.Bi_c_vol * (J - W)

    # heat flux
    h = aux["h"]
    theta_t = (-(h[1:] - h[:-1]) / dx + p.Da * J * aux["R"]) / aux["C_ref"] \
...
def conservation_diagnostics(data: Dict, p: Params, late_frac: float = 0.60) -> Dict:
    p = finalize_params(p)
    t = np.asarray(data["t"])
    J = np.asarray(data["J"])
    ...
    return {
        "reactant_balance_residual": _integrated_residual(reactant_inventory, reactant_rhs, all_sl),
        "heat_balance_residual": _integrated_residual(heat_inventory, heat_rhs, all_sl),
        "reactant_balance_residual_late": _integrated_residual(reactant_inventory, reactant_rhs, late_sl),
        "heat_balance_residual_late": _integrated_residual(heat_inventory, heat_rhs, late_sl),
        "max_enthalpy_advection_to_conduction_ratio": float(np.nanmax(enthalpy_adv_ratio)) if nt else float("nan"),
...
def parse_args():
    ...
    ap.add_argument("--model-version", default="v0",
                    choices=["v0", "model_v0", "v1_first_principles",
                             "model_v1_first_principles",
                             "v1_effective_chi_optional",
                             "model_v1_effective_chi_optional",
                             "v1_metric_corrected_optional",
                             "model_v1_metric_corrected_optional"])
    ap.add_argument("--closure-form", default=None,
                    choices=["power", "mackie_meares", "free_volume"])
...
def main():
    ...
    if args.closure_form is not None: overrides["closure_form"] = args.closure_form
    if args.A_min is not None: overrides["A_min"] = args.A_min
    if args.D_min is not None: overrides["D_min"] = args.D_min
